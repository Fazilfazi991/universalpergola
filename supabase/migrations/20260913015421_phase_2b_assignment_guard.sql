-- Keep CRM ownership restricted to active Management or Sales profiles and
-- ensure follow-up tasks cannot be relinked to a different customer.
...
begin
  ...
  if tg_table_name = 'tasks' and new.kind = 'enquiry_follow_up'::public.task_kind then
...
create trigger customers_validate_assignee
before insert or update of assigned_to on public.customers
for each row execute function private.validate_crm_assignee();

create trigger enquiries_validate_assignee
before insert or update of assigned_to on public.enquiries
for each row execute function private.validate_crm_assignee();

create trigger tasks_validate_crm_assignment
before insert or update of assigned_to, enquiry_id, customer_id, kind on public.tasks
for each row execute function private.validate_crm_assignee();
