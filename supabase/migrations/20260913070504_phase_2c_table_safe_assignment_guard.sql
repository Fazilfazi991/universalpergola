-- Keep table-specific record fields inside their table branch. PostgreSQL can
-- resolve both sides of a boolean expression for a polymorphic trigger record,
-- so `tg_table_name = 'tasks' and new.kind ...` is not table-safe.
...
begin
  if tg_table_name in ('customers', 'enquiries') then
    ...
    if new.assigned_to is not null then
      ...
      if not assignee_valid then
        raise exception 'Choose an active staff member permitted for this task';
      end if;
    end if;
