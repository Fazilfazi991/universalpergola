      ...
      <section className="relative hidden overflow-hidden bg-ink p-12 text-white lg:flex lg:flex-col lg:justify-between">
        ...
        <div className="relative max-w-xl pb-8">
          ...
          <p className="mt-6 max-w-md text-base leading-7 text-white/55">
            One protected workspace for catalogue, customers, site work, quotations, projects, and payments.
          </p>
        </div>
      ...
      <section className="flex min-w-0 items-center justify-center px-4 py-12 sm:px-8">
        <div className="w-full max-w-md">
          ...
          <div className="mt-12 lg:mt-0">
            ...
            <h2 className="mt-2 text-3xl font-semibold tracking-[-0.045em] text-graphite">
              Welcome back
            </h2>
            <p className="mt-2 text-sm leading-6 text-stone">
              Sign in with your Universal Pergola staff account.
            </p>
          </div>
          <div className="mt-6 space-y-3">
            {!configured && (
              <StatusNotice tone="error" title="Setup required">
                <p>
                  Copy <code>.env.example</code> to <code>.env.local</code> and add the Supabase project settings.
                </p>
              </StatusNotice>
            )}
            {reason === "profile" && (
              <StatusNotice tone="error" title="Account unavailable">
                <p>Your staff profile is missing or inactive. Ask an administrator to review the account.</p>
              </StatusNotice>
            )}
          </div>
