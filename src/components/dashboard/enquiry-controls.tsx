...
export function EnquiryEditor({ enquiry, role }: { enquiry: EnquiryDetail; role: AppRole }) {
  ...
  const [leadSource, setLeadSource] = useState(enquiry.lead_source || "Other");
  return <form action={action} className="space-y-4">
    {state.message && <p className={`text-sm ${state.status === "error" ? "text-red-700" : "text-emerald-700"}`} role="status">{state.message}</p>}
    <div className="grid gap-4 sm:grid-cols-2"><label className="grid gap-2 text-sm font-medium">Status<select className={input} name="status" defaultValue={enquiry.status}>{ENQUIRY_STATUSES.map((item) => <option key={item} value={item}>{ENQUIRY_STATUS_LABELS[item]}</option>)}</select></label><label className="grid gap-2 text-sm font-medium">Priority<select className={input} name="priority" defaultValue={enquiry.priority}>{LEAD_PRIORITIES.map((item) => <option key={item} value={item}>{PRIORITY_LABELS[item]}</option>)}</select></label><label className="grid gap-2 text-sm font-medium">Lead source<select className={input} name="lead_source" value={leadSource} onChange={(event) => setLeadSource(event.target.value)}>{ENQUIRY_SOURCES.map((item) => <option key={item}>{item}</option>)}</select></label><input type="hidden" name="source" value={leadSource} /><label className="grid gap-2 text-sm font-medium">Follow-up date<input className={input} name="follow_up_at" type="datetime-local" defaultValue={localDateTime(enquiry.follow_up_at)} /></label>{leadSource === "Referral Person" && <label className="grid gap-2 text-sm font-medium sm:col-span-2">Referred by<input className={input} name="referred_by" defaultValue={enquiry.referred_by || ""} required maxLength={160} /></label>}{leadSource === "Other" && <label className="grid gap-2 text-sm font-medium sm:col-span-2">Source details<input className={input} name="lead_source_detail" defaultValue={enquiry.lead_source_detail || ""} required maxLength={300} /></label>}{leadSource !== "Referral Person" && <input type="hidden" name="referred_by" value="" />}{leadSource !== "Other" && <input type="hidden" name="lead_source_detail" value="" />}<label className="grid gap-2 text-sm font-medium sm:col-span-2">Next action<input className={input} name="next_action" defaultValue={enquiry.next_action || ""} maxLength={300} /></label><label className="grid gap-2 text-sm font-medium sm:col-span-2">Internal notes<textarea className={`${input} min-h-28 py-3`} name="internal_notes" defaultValue={enquiry.internal_notes || ""} maxLength={6000} /></label></div><input type="hidden" name="assigned_to" value={enquiry.assigned_to || ""} /><button disabled={pending} className="min-h-11 rounded-md bg-graphite px-4 text-sm font-semibold text-white disabled:opacity-50">{pending ? "Saving…" : "Save enquiry"}</button>{role === "sales" && <p className="text-xs text-stone">Management controls salesperson assignment.</p>}
  </form>;
}
