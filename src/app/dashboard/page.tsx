      ...
      {crm ? (
        <>
          ...
          <section className="rounded-lg border border-line bg-paper">
            <div className="flex items-end justify-between gap-4 border-b border-line px-4 py-4 sm:px-5"><div><h2 className="text-base font-semibold">Reminder attention</h2><p className="mt-1 text-sm text-stone">Overdue, today, upcoming, and enquiries without a future follow-up.</p></div><Link href="/dashboard/tasks" className="min-h-10 py-2 text-sm font-medium text-brass-dark">Open reminders</Link></div>
            <div className="grid divide-y divide-line lg:grid-cols-2 lg:divide-x lg:divide-y-0"><div className="p-4 sm:p-5"><h3 className="text-sm font-semibold">Scheduled</h3><div className="mt-3 space-y-3">{crm.reminderAttention.length ? crm.reminderAttention.map((item) => <Link key={item.id} href={item.project_id ? `/dashboard/projects/${item.project_id}` : item.enquiry_id ? `/dashboard/enquiries/${item.enquiry_id}` : "/dashboard/tasks"} className="block rounded-md border border-line px-3 py-3 hover:border-brass"><div className="flex justify-between gap-3"><span className="truncate text-sm font-medium">{item.title}</span><span className={`shrink-0 text-xs ${item.bucket === "overdue" ? "text-red-700" : "text-stone"}`}>{item.bucket === "today" ? "Today" : item.bucket === "overdue" ? "Overdue" : "Upcoming"}</span></div><p className="mt-1 text-xs text-stone">{item.reminder_type || "Follow-up"} · {formatDate(item.due_at, true)}</p></Link>) : <p className="text-sm text-stone">No active reminders.</p>}</div></div><div className="p-4 sm:p-5"><h3 className="text-sm font-semibold">No follow-up scheduled</h3><div className="mt-3 space-y-3">{crm.noFollowUp.length ? crm.noFollowUp.map((item) => <Link key={item.id} href={`/dashboard/enquiries/${item.id}`} className="block rounded-md border border-line px-3 py-3 hover:border-brass"><p className="text-sm font-medium">{enquiryReference(item.enquiry_number)}</p><p className="mt-1 truncate text-xs text-stone">{item.customer?.name || item.subject || "Active enquiry"}</p></Link>) : <p className="text-sm text-stone">Every active enquiry has a future follow-up.</p>}</div></div></div>
          </section>
          <section className="rounded-lg border border-line bg-paper">
