...
import { AlertTriangle, ArrowLeft, Check, Download, Link2, Trash2 } from "lucide-react";
...
export default async function ProjectPage({ params, searchParams }: PageProps<"/dashboard/projects/[id]">) {
  ...
  if (!project) notFound();
  const [workspace, options] = await Promise.all([getProjectWorkspace(id), profile.role === "admin" ? getProjectOptions() : Promise.resolve({ customers: [], staff: [], templates: [] })]);
