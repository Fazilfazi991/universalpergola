...
export default async function ProjectPage({ params, searchParams }: PageProps<"/dashboard/projects/[id]">) {
  ...
  const checklist = workspace.tasks.filter((task) => task.completion_checklist_key);
  const checklistComplete = checklist.filter((task) => task.status === "completed").length;
    ...
    <div className="space-y-7">
      ...
      <section className="rounded-lg border border-line bg-paper" aria-label="Completion summary">
        ...
        <dl className="grid grid-cols-2 md:grid-cols-4 xl:grid-cols-8">
          {[["Project",project.project_number],["Customer",project.customer?.name||"—"],["Completion",formatDate(project.actual_completion_date)],["Stage",currentStage?.name||"Complete"],["Handover",handoverStatusLabel(project.handover_status)],["Value",formatMoney(project.project_value,project.currency)],["Checklist",`${checklistComplete}/${checklist.length}`],["Open tasks",workspace.tasks.filter((task)=>!task.completion_checklist_key&&!["completed","cancelled"].includes(task.status)).length]].map(([label,value])=><div key={String(label)} className="min-w-0 border-b border-line px-4 py-4 odd:border-r md:border-r md:[&:nth-child(4n)]:border-r-0 xl:border-b-0 xl:[&:nth-child(4n)]:border-r xl:last:border-r-0"><dt className="text-[10px] uppercase tracking-[0.13em] text-stone">{label}</dt><dd className="mt-1 truncate text-sm font-semibold">{value}</dd></div>)}
        </dl>
        {finance ? <div className="grid grid-cols-2 gap-3 border-t border-line px-4 py-3 text-xs sm:flex sm:justify-end sm:px-5"><span>Received <strong>{formatMoney(finance.received,finance.currency)}</strong></span><span>Outstanding <strong className={finance.outstanding>0?"text-amber-800":""}>{formatMoney(finance.outstanding,finance.currency)}</strong></span></div> : null}
      </section>
      <div className="grid gap-7 xl:grid-cols-[minmax(0,1.55fr)_minmax(19rem,0.65fr)]">
        <div className="space-y-7">
          ...
          <section className="border-y border-line bg-paper px-4 py-5 sm:rounded-lg sm:border sm:p-6">
            <div className="flex items-end justify-between gap-4"><div><h2 className="text-lg font-semibold">Completion checklist</h2><p className="mt-1 text-sm text-stone">A lightweight handoff record; incomplete items warn but do not replace completion rules.</p></div><strong className="text-sm">{checklistComplete}/{checklist.length}</strong></div>
            <div className="mt-5 divide-y divide-line">{checklist.map((item)=><article key={item.id} className="py-4 first:pt-0 last:pb-0"><div className="flex items-start gap-3"><span className={`mt-0.5 grid size-5 shrink-0 place-items-center rounded-full border ${item.status==="completed"?"border-emerald-600 bg-emerald-600 text-white":"border-line bg-limestone"}`}>{item.status==="completed"?<Check size={12}/>:null}</span><div className="min-w-0 flex-1"><p className="text-sm font-medium">{item.title}</p>{item.status==="completed"?<p className="mt-1 text-xs text-stone">Completed {formatDate(item.completed_at,true)} by {item.completer?.full_name||"Team"}{item.completion_note?` · ${item.completion_note}`:""}</p>:null}{canOperate?<form action={updateCompletionChecklistAction} className="mt-3 flex flex-col gap-2 sm:flex-row"><input type="hidden" name="project_id" value={project.id}/><input type="hidden" name="key" value={item.completion_checklist_key||""}/><input type="hidden" name="completed" value={String(item.status!=="completed")}/><input name="note" maxLength={2000} defaultValue={item.completion_note||""} className="min-h-10 min-w-0 flex-1 rounded-md border border-line bg-paper px-3 text-sm" placeholder="Optional note"/><button className={`min-h-10 shrink-0 text-sm font-semibold ${item.status==="completed"?"text-brass-dark":"text-emerald-700"}`}>{item.status==="completed"?"Reopen":"Mark complete"}</button></form>:null}</div></div></article>)}</div>
          </section>
          <section className="border-y border-line bg-paper px-4 py-5 sm:rounded-lg sm:border sm:p-6">
            <div className="flex items-end justify-between gap-4"><div><h2 className="text-lg font-semibold">Tasks</h2><p className="mt-1 text-sm text-stone">Execution actions linked to the project or a stage.</p></div><span className="text-xs text-stone">{workspace.tasks.filter((task) => !task.completion_checklist_key && !["completed", "cancelled"].includes(task.status)).length} open</span></div>
            <div className="mt-5 divide-y divide-line">{workspace.tasks.filter((task)=>!task.completion_checklist_key).length ? workspace.tasks.filter((task)=>!task.completion_checklist_key).map((task) => (
        ...
        <aside className="space-y-5">
          ...
          {feedback ? <section className="rounded-lg border border-line bg-paper p-5"><div className="flex items-center justify-between gap-3"><h2 className="text-base font-semibold">Customer feedback</h2><span className={`rounded-sm border px-2 py-1 text-xs ${feedbackStatusClass(feedback.status)}`}>{FEEDBACK_STATUS_LABELS[feedback.status]}</span></div><dl className="mt-4 grid gap-3 text-sm"><div><dt className="text-xs text-stone">Rating</dt><dd className="mt-1 font-semibold">{ratingLabel(feedback.customer_rating)}</dd></div>{feedback.customer_comments?<div><dt className="text-xs text-stone">Customer comment</dt><dd className="mt-1 whitespace-pre-wrap leading-6">{feedback.customer_comments}</dd></div>:null}<div><dt className="text-xs text-stone">Testimonial permission</dt><dd className="mt-1">{feedback.permission_to_publish_testimonial?"Recorded":"Not given"}</dd></div></dl><Link href="/dashboard/feedback" className="mt-4 inline-flex min-h-10 items-center text-sm font-semibold text-brass-dark">Manage feedback →</Link></section> : (profile.role==="admin"||profile.role==="sales") && project.status==="completed" ? <section className="rounded-lg border border-dashed border-line bg-paper p-5"><h2 className="text-base font-semibold">Customer feedback</h2><p className="mt-2 text-sm text-stone">Feedback has not been requested.</p><Link href="/dashboard/feedback" className="mt-3 inline-flex min-h-10 items-center text-sm font-semibold text-brass-dark">Create request →</Link></section> : null}
