...
export default async function FeedbackAdminPage({ searchParams }: PageProps<"/dashboard/feedback">) {
  ...
  return (
    <div className="space-y-7">
      ...
      {isDemoMode() && <DemoDisabledNotice>Feedback requests, moderation, and recording are disabled in the public demo.</DemoDisabledNotice>}
      ...
      <details className={`rounded-lg border border-line bg-paper ${isDemoMode() ? "hidden" : ""}`} open={!rows.length}>
      ...
      <details className={`rounded-lg border border-line bg-paper ${isDemoMode() ? "hidden" : ""}`}>
